...
const setBigintParser = Symbol('setBigintParser');
...
class PostgresProvider extends StorageProvider {
  ...
  [setBigintParser](callback) {
    this.pool.query('SELECT \'bigint\'::regtype::oid', (err, res) => {
      if (err) {
        callback(err);
        return;
      }
      if (res.rows.length === 0) {
        callback(new GSError(errorCodes.INTERNAL_ERROR));
        return;
      }
      pg.types.setTypeParser(res.rows[0].oid, val => new common.Int64(val));
      callback(null);
    });
  }
  ...
  set(obj, callback) {
    ...
    this.getCategoryById(obj.Id, (err, category) => {
      ...
      const categoryDefinition =
        this.gs.schema.categories.get(category).definition;
      const toValidate = copyValueForValidation(categoryDefinition, obj);
      ...
      this.pool.connect((err, client, done) => {
        ...
        metasync.sequential([
          ...
          (ctx, cb) => {
            updateRecord(category, obj, client, err => { cb(err); });
          },
        ], (err, ctx) => {
          if (err) {
