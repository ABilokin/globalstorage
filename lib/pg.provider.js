...
const selectWithId = Symbol('selectWithId');
...
class PostgresProvider extends StorageProvider {
  ...
  [selectWithId](categories, callback) {
    const results = {};
    metasync.each(categories, (cat, callback) => {
      this.select(cat, {}).fetch((err, rows) => {
        if (err) {
          callback(err);
        } else {
          results[cat] = rows;
  ...
  set(obj, callback) {
    ...
    this.getCategoryById(obj.Id, (err, category) => {
      ...
      const setQuery = `UPDATE ${escapeIdentifier(category)}` +
        ` SET (${fields.join(', ')}) =` +
        ` ROW (${generateQueryParams(fields.length)})`;
